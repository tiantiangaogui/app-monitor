...
一个会自动更新、部署在云端的 App 包体在线状态监控页。**每4小时自动核验** App Store 上各 App 的在线状态与包体信息，并把最新结果发布为网站；**发现 App 下架时自动发送邮件告警**。
...
| `.github/workflows/update.yml` | 每4小时定时任务：自动运行检测脚本（含下架邮件通知）并把最新 `data.json` 推回仓库 |
...
3. **确认自动更新已生效**
   - 仓库 → `Actions` 标签，应能看到名为「包体状态核验（每4小时）」的 workflow。
   - 首次进入后可点该 workflow → 右侧 **Run workflow** 手动跑一次，验证流程正常（约 30 秒）。
   - 之后 workflow 会按计划**每4小时自动运行**：核验各 App 状态 → 若 `data.json` 有变化则自动提交推送 → GitHub Pages 自动重新发布。全程无需人工干预。
...
- **方式二（已部署后、不 clone 本地）**：直接在 GitHub 仓库网页编辑 `apps.json`，加一行：
  ...
  ```
  提交后到 Actions 里手动 Run 一次「包体状态核验（每4小时）」即可立即生效（之后每4小时自动核验也会包含它）。
...
**手动立即核验**：进入 Actions → 选中「包体状态核验（每4小时）」→ Run workflow，即可马上跑一次检测。

## 四、常见问题

- **网站显示"本地预览模式"**：说明当前是用浏览器直接打开本地文件，`data.json` 读取不到。部署到 GitHub Pages 后即为自动模式。
- **多久更新一次**：默认每4小时整点（北京时间）自动核验。想改频率，编辑 `.github/workflows/update.yml` 里的 `cron` 表达式（`0 */4 * * *` 为每4小时一次，UTC 时间；`0 * * * *` 为每小时一次）。
- **数据没变化时会不会白跑**：不会。脚本每4小时都会核验，但只有结果真的变化才提交新数据，无变化则跳过提交。
