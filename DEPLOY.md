...
把「App 包体信息监控台」部署到云端，实现**每4小时自动核验 App 在线状态 + 下架邮件告警 + 网站自动更新**，全程免费，约 5 分钟。
...
1. 进入仓库 → 点击顶部 **Actions**
2. 左侧应能看到名为 **「包体状态核验（每4小时）」** 的 workflow
...
> 验证成功后，该 workflow 会按计划**每4小时自动运行**：核验各 App → 有变化则自动更新数据并推送 → GitHub Pages 自动重新发布。全程无需人工干预。
...
4. 邮件触发逻辑：**只有当某个包"从在线变为下架"时**才会发通知（每4小时检测，历史已下架的包不会重复骚扰）；通知里包含包名、Bundle ID、开发者、商店链接和检测时间。
...
- **部署后免本地**：在 GitHub 网页直接编辑 `apps.json` 加一行，提交后在 Actions 手动 Run 一次「包体状态核验（每4小时）」即生效。
...
```yaml
schedule:
  - cron: '0 */4 * * *'   # 每 4 小时整点运行（UTC 时间；北京时间 = UTC + 8）
```

- `'0 */4 * * *'` = 每 4 小时一次（当前默认，UTC 0/4/8/12/16/20 时 = 北京 8/12/16/20/0/4 时）
- 想每小时一次 → `'0 * * * *'`
- 想每天一次 → `'15 0 * * *'`（UTC 00:15 = 北京 08:15）
- 想每 30 分钟一次 → `'*/30 * * * *'`

> 注意：GitHub Actions 免费账户的 cron 调度可能有几分钟到数小时的延迟，属正常现象（不保证严格按点）；每4小时运行对免费额度（约 6×30=180 次/月）完全够用。
...
| 定时任务没到点执行 | GitHub Actions 的 cron 最小粒度是 5 分钟，且实际执行时间可能有几分钟甚至数小时延迟，属正常现象（不保证严格按点触发） |
...
```
每4小时整点
   └─ GitHub Actions 触发「包体状态核验（每4小时）」workflow
        └─ 运行 check_status.py
